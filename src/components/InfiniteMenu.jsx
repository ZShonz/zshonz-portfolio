...
void main() {
    ...
    vec4 projectColor = texture(uTex, st);
    float luminance = dot(projectColor.rgb, vec3(0.299, 0.587, 0.114));
    vec3 mutedColor = mix(vec3(luminance * 0.58), projectColor.rgb * 0.62, 0.24);
    float focus = smoothstep(0.45, 0.86, vAlpha);
    outColor = vec4(mix(mutedColor, projectColor.rgb, focus), vAlpha);
}
`;
...
function createProjectPositions(count, radius) {
  if (count <= 1) return [vec3.fromValues(0, 0, radius)];

  const positions = [];
  const goldenAngle = Math.PI * (3 - Math.sqrt(5));
  const latitudeLimit = 0.72;

  for (let index = 0; index < count; index += 1) {
    const y = latitudeLimit - (index / (count - 1)) * latitudeLimit * 2;
...
class ProjectCardGeometry extends Geometry {
  constructor() {
    super();
    const points = [
    ...
    points.forEach(([x, y]) => {
      ...
      this.lastVertex.uv[1] = (y + 0.7) / 1.4;
    });

    for (let i = 1; i <= points.length; i += 1) {
      this.addFace(0, i, i === points.length ? 1 : i + 1);
    }
  }
}
...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    ...
    this.discGeo = new ProjectCardGeometry();
    ...
    this.instancePositions = createProjectPositions(Math.max(1, this.items.length), this.SPHERE_RADIUS);
  ...
  #animate(deltaTime) {
    ...
    const scale = Math.max(0.28, Math.min(0.52, 0.76 / Math.pow(this.DISC_INSTANCE_COUNT, 0.22)));
  ...
  #updateProjectionMatrix(gl) {
    ...
    const height = this.SPHERE_RADIUS * 0.44;
...
export default function InfiniteMenu({ items = [], scale = 1.0, backgroundColor = '#0b0d0e', onActiveItemChange, onItemSelect }) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      ...
      onActiveItemChange?.(nextItem, itemIndex);
    };
  ...
  return (
    ...
    >
      <canvas
        ...
        ref={canvasRef}
        tabIndex="0"
        data-project-count={items.length}
        aria-label={`拖动浏览 ${items.length} 张作品项目卡片`}
      />

      {activeItem && (
        <button type="button" onClick={handleButtonClick} aria-label={`查看 ${activeItem.title}`} className={`action-button ${isMoving ? 'inactive' : 'active'}`}>
